# Lambda function to detecth labels in image using Amazon Rekognition

import logging
import boto3 from botocore.exceptions
import ClientError
import os from urllib.parse
import unquote_plus from boto3.dynamodb.conditions
import Key, Attr
...
def rekFunction(ourBucket, ourKey):
  # Clean the string to add the colon back into requested name which was substituted by Amplify Library
  ...
  # Try and retrieve labels from Amazon Rekognition, using the confidence level we set in the minConfidence var
  try:
    detectLabelsResults = rekognition_client.detect_labels(Image={
      'S3Object': { 
        'Bucket': ourBucket, 
        'Name': safeKey 
      }},
